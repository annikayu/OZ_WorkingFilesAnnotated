#!/usr/bin/env python3
"""
Maryland Opportunity Zones Change Map Generator
================================================
Fetches ACS 5-year estimates (2017 → 2023) for Maryland OZ 1.0 census tracts
and produces a standalone maryland_oz_map.html for embedding in ArcGIS Story Maps.
...
boundaries. Most Maryland tracts are identical; a small number that were split/merged
...
# uses to identify each state — Maryland is 24. YEAR_START and YEAR_END mark the
...
YEAR_START     = 2017
YEAR_END       = 2023
OUTPUT_FILE    = "maryland_oz_map.html"
...
# an ArcGIS feature service. We query it filtered to Maryland (STATE = '24') and
...
def fetch_oz_geoids():
    print("▶  Fetching Maryland OZ tract GEOIDs from HUD…")
    ...
    while True:
        ...
        offset += 1000
        time.sleep(0.25)
    print(f"   → {len(geoids)} Maryland OZ tracts found")
    if not geoids:
        sys.exit("ERROR: No Maryland OZ tracts returned from HUD service.")
    return geoids
...
# the six indicator variables for every census tract in Maryland. We intentionally
...
def fetch_acs(year):
    print(f"▶  Fetching {year} ACS 5-year estimates for all Maryland tracts…")
...
def fetch_geometry(oz_set):
    print("▶  Downloading Maryland census tract boundaries…")
    candidate_urls = [
        "https://www2.census.gov/geo/tiger/GENZ2019/json/cb_2019_24_tract_500k.json",
        "https://raw.githubusercontent.com/uscensusbureau/citysdk/master/v2/GeoJSON/500k/2019/24/tract.json",
    ]
    geojson = None
    ...
    if not geojson:
        sys.exit("ERROR: Could not download Maryland tract boundaries. "
...
<head>
  ...
  <title>Maryland Opportunity Zones: 2017–2023 Change</title>
...
<div id="app">
  <div id="header">
    <div id="header-text">
      <h1>Maryland Opportunity Zones &mdash; Change Over Time</h1>
 ...
 * on Maryland and set a bounding box covering the continental United States so
...
# Maryland OZ tract GEOIDs from HUD. Then we pull ACS survey data from the Census
...
def main():
    print("\n══ Maryland OZ Change Map Generator ══\n")
